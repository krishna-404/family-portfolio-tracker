...
export const journalEntryCreateInputZod = journalEntryMandatoryZod
	...
	.extend(journalEntryOptionalZod.omit({ teamId: true }).partial().shape)
...
export const journalEntryUpdateInputZod = journalEntryMandatoryZod
	.extend(journalEntryOptionalZod.omit({ teamId: true }).shape)
	.partial();
...
export const openapiJournalEntryCreateInputZod = openapiRequestInputZod.extend({
	// External API callers must supply the author user id explicitly —
	// no session is available on the openapi flow to attribute the row.
	data: journalEntryCreateInputZod.extend({ authorUserId: z.uuid() })
});
